...
function CreateDropdown() {
  ...
  const items = [
    { icon: 'message',  label: 'New Message',      sub: 'Direct message',                                                   onClick: () => openModal('newMessage'),      enabled: true },
    { icon: 'users',    label: 'New Group',        sub: 'Private group chat',                                               onClick: () => openModal('newGroup'),        enabled: true },
    { icon: 'hash',     label: 'New Channel',      sub: 'Public or private',                                                onClick: () => openModal('newChannel'),      enabled: true },
    { icon: 'calendar', label: 'Schedule Meeting', sub: activeChannelId ? 'In current channel' : 'Open a channel first', onClick: () => openModal('scheduleMeeting'), enabled: !!activeChannelId },
  ];
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button
        ...
        <Icon name="add" size={12} /> New <span style={{ opacity: .8, display: 'inline-flex' }}><Icon name="chevron-down" size={12} /></span>
      </button>
      {open && (
        <div style={{
          ...
          {items.map((it, i) => (
            <div
              ...
              <span style={{ width: 20, display: 'inline-flex', justifyContent: 'center', flexShrink: 0 }}><Icon name={it.icon as IconName} size={16} /></span>
...
function StatusChip() {
  ...
  return (
    <button onClick={() => openModal('setStatus')} style={{ ...btn(), gap: 5 }} title="Change status">
      <Icon name="circle" size={8} color={color} fill={color} />
      {label}
      <span style={{ opacity: .6, marginLeft: 2, display: 'inline-flex' }}><Icon name="chevron-down" size={11} /></span>
    </button>
  );
}
    ...
    <div style={{
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', gap: 5, alignItems: 'center' }}>
        ...
        <button style={btn()} onClick={() => window.open(import.meta.env.VITE_PCI_URL || 'https://ias-app.planetsg.com', '_blank')}
          ...
          <Icon name="external-link" size={13} /> Open PCI
        </button>

        {/* Settings cog */}
        <div ref={settingsRef} style={{ position: 'relative' }}>
          <button style={btn(showSettings)} onClick={() => setShowSettings(o => !o)} title="Settings"><Icon name="settings" size={14} /></button>
          {showSettings && (
            <div style={{
              ...
              <SettingsItem icon="user"     label="My Profile"         onClick={() => { setShowSettings(false); }} />
              <SettingsItem icon="bell"     label="Notifications"      onClick={() => { setShowSettings(false); }} />
              <SettingsItem icon="palette"  label="Appearance"         onClick={() => { setShowSettings(false); }} />
              <SettingsItem icon="keyboard" label="Keyboard Shortcuts" onClick={() => { setShowSettings(false); }} />
              <div style={{ height: 1, background: 'var(--neutral-light-active)' }} />
              <SettingsItem icon="logout"   label="Log out"            onClick={() => { setShowSettings(false); logout(); }} danger />
            </div>
          )}
        </div>
      </div>
...
function SettingsItem({ icon, label, onClick, danger }: { icon: IconName; label: string; onClick: () => void; danger?: boolean }) {
  return (
    <div onClick={onClick}
      ...
      onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
      <span style={{ width: 18, display: 'inline-flex', justifyContent: 'center' }}><Icon name={icon} size={14} /></span>
      <span>{label}</span>
    </div>
  );
}
